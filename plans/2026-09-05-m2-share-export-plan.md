...
- [x] F5.1 Migrasi `bill_share_tokens(id UUID PK, bill_id FK, token_hash TEXT UNIQUE, created_by UUID, created_at, expires_at, revoked_at NULL, last_viewed_at NULL)`: index `(bill_id, revoked_at, expires_at)`; `ENABLE RLS` + SATU policy SELECT owner (`load()` butuh ini; RLS deny-by-default); `REVOKE ALL FROM PUBLIC, anon, authenticated` + GRANT SELECT eksplisit (standar 2026-08-30). Fungsi `SECURITY DEFINER SET search_path = ''` + `public.` qualified (insiden `ensure_profile` 2026-05-02).
...
- 2026-09-06 (build) — Implementasi selesai: F5.1 migration `20260906120000_bill_share_tokens.sql` + owner SELECT policy (tambahan vs plan: `load()` butuh policy, RLS deny-by-default); F5.2 RPC sesuai plan (create/revoke/resolve, `SECURITY DEFINER`+`search_path=''`); F5.3 `_ShareLinkSection` di bill_detail (create/salin via `Share.shareUri`/clipboard + expiry + revoke + Free-limit message); F5.4 `SharedBillScreen` read-only + rute `/share/:token` + exemption semua gate + `DeepLinkHandler.parseShareToken` + intent-filter host `share` (iOS: scheme global, tanpa plist change); F5.5 ARB ID+EN; F7 `BillXlsxExporter` (2 sheet, sel numerik raw, sanitasi nama sheet, guard `'=+-@`, header-only bila kosong, `fileName` slug+billId8) + gate Plus identik CSV/PDF + mime xlsx; F5.6/F7.4 test (token/hash, parse/consume, fromJson, parity CSV↔XLSX, header-only, formula, unicode, bank-only-if-complete, filename unik, sanitasi). Privacy EN+ID + release-play-store Data safety disinkron. Verifikasi: analyze 0 error, test 540 passed (525+15).
